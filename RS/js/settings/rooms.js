...
function room_variations_change(){ // Show / hide additional prices form
  ...
  if(val == 1){
    ...
    $("#room_settings_variations_default").append(`<option value=1> 1 osoba </option>`);
    $("#room_settings_variations_default").append(`<option value=${num}> ${num} osoba </option>`);
...
$("#room_settings_confirm").click(function(){
  ...
  $.ajax({
    ...
    success: function(rezultat){
      ...
      let change_text = id == -1 ? `Dodata jedinica ${name} ` : `Izmenjena jedinica ${name}`; // Edit or insert
