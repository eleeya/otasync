...
$(document).ready(function(){
      ...
      $(`#${row_id}`).after(`
        ...
        <input type='hidden' id='edit_promocode_id' value='${id}'>
          ...
          <div class='flex_center'>
            <button class='cancel_button' id='edit_promocode_cancel'> CANCEL </button>
            <button class='confirm_button' id='edit_promocode_confirm'> SAVE </button>
          </div>
        </div>`);
      // Add values
  ...
  $("body").on("click", "#edit_promocode_confirm", function(){
    ...
    $.ajax({
      ...
      success: function(rezultat){
        ...
        add_change(`Edited promo code ${$("#edit_promocodeName").val()}`, sve.data.id); // Add changelog
        get_promocodes(); // Refresh data
      },
      error: function(xhr, textStatus, errorThrown){
        $(".button_loader").removeClass("button_loader");
  ...
  $("#promocodes_list").on("click", ".delete", function(){ // Show dialog and delete
    ...
    id = id[id.length - 1];
    let promocode = promocodes_map[id];
    if(confirm(`Are you sure you want to the delete promo code ${promocode.name}`)){
      ...
      $.ajax({
        ...
        success: function(rezultat){
          ...
          add_change(`Deleted promo code ${promocode.name}`, sve.data.id); // Add changelog
          get_promocodes(); // Refresh data
        },
        error: function(xhr, textStatus, errorThrown){
          window.alert("An error occured. " + xhr.responseText);
  ...
  $("#new_policy_confirm").click(function(){
    ...
    $.ajax({
      ...
      success: function(rezultat){
        ...
        add_change(`Added cancellation policy ${$("#policyName").val()}`, sve.data.id); // Add changelog
      ...
      $(`#${row_id}`).after(`
        <div id='edit_policy_container'>
          ...
          <div class='flex_between edit_policy_free_days'>
            <div> Days for free cancellation </div>
          ...
          <div class='flex_center'>
            <button class='cancel_button' id='edit_policy_cancel'> CANCEL </button>
            <button class='confirm_button' id='edit_policy_confirm'> SAVE </button>
          </div>
        </div>`);
      // Add values
  ...
  $("body").on("click", "#edit_policy_confirm", function(){
    ...
    $.ajax({
      ...
      success: function(rezultat){
        ...
        add_change(`Edited cancellation policy ${$("#edit_policyName").val()}`, sve.data.id); // Add changelog
        get_policies(); // Refresh data
      },
      error: function(xhr, textStatus, errorThrown){
        $(".button_loader").removeClass("button_loader");
  ...
  $("#policies_list").on("click", ".delete", function(){ // Show dialog and delete
    ...
    id = id[id.length - 1];
    let policy = policies_map[id];
    if(confirm(`Are you sure you want to delete cancellation policy ${policy.name}`)){
      ...
      $.ajax({
        ...
        success: function(rezultat){
          ...
          add_change(`Deleted cancellation policy ${policy.name}`, sve.data.id); // Add changelog
          get_policies(); // Refresh data
        },
        error: function(xhr, textStatus, errorThrown){
          window.alert("An error occured. " + xhr.responseText);
...
function display_promocodes(promocodes){
  ...
  for(let i=0;i<promocodes.length;i++){
    ...
    if(promocode.type == "percentage")
      ...
      amount += " EUR";
    var promocode_edit = `<div class='list_action'><img class='list_action_icon edit' title='Edit'> </div>`;
    var promocode_delete = `<div class='list_action'><img class='list_action_icon delete' title='Delete'> </div>`;
...
function display_policies(){
  ...
  for(let i=0;i<policies_list.length;i++){
    ...
    var policy_edit = `<div class='list_action'><img class='list_action_icon edit' title='Edit'> </div>`;
    var policy_delete = `<div class='list_action'><img class='list_action_icon delete' title='Delete'> </div>`;
