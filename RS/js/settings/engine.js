  ...
  $("body").on("click", "#edit_promocode_confirm", function(){
    ...
    $.ajax({
      ...
      success: function(rezultat){
        ...
        add_change(`Izmenjen promo kod ${$("#edit_promocodeName").val()}`, sve.data.id); // Add changelog
        get_promocodes(); // Refresh data
      },
      error: function(xhr, textStatus, errorThrown){
        $(".button_loader").removeClass("button_loader");
  ...
  $("body").on("click", "#edit_policy_confirm", function(){
    ...
    $.ajax({
      ...
      success: function(rezultat){
        ...
        add_change(`Izmenjena politika otkazivanja ${$("#edit_policyName").val()}`, sve.data.id); // Add changelog
        get_policies(); // Refresh data
      },
      error: function(xhr, textStatus, errorThrown){
        $(".button_loader").removeClass("button_loader");
...
function display_promocodes(promocodes){
  ...
  for(let i=0;i<promocodes.length;i++){
    ...
    if(promocode.type == "percentage")
      ...
      amount += " EUR";
    var promocode_edit = `<div class='list_action'><img class='list_action_icon edit' title='Izmeni'> </div>`;
...
function display_policies(){
  ...
  for(let i=0;i<policies_list.length;i++){
    ...
    var policy_edit = `<div class='list_action'><img class='list_action_icon edit' title='Izmeni'> </div>`;
