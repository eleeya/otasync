...
// Gets basic login, list of properties, access, account settings
function get_login(){
  ...
  $.ajax({
    ...
    success: function(rezultat){
      ...
      if(sve.status !== "ok"){
        ...
        $("#login_error").text("Invalid username/password.");
        return;
      }
      // Save data
      main_key = sve.key;
...
// Gets login data with session key instead of username/pass
function get_login_session(){
  $.ajax({
    ...
    success: function(rezultat){
      ...
      if(sve.status !== "ok"){
        $(".button_loader").removeClass("button_loader");
        $("#login_error").text("Session expired.");
        return;
      }
      // Save data
      main_key = sve.key;
...
// Adds user settings values and inits dates, only called on login
function display_user_info(data){
  ...
  $("#account_invoice_due").val(data.invoice_issued).change();
...
// Gets all data for the property, called on login and proprety change
function get_basics(){
  $.ajax({
...
// Displays data fetched by get_basics()
function display_property_info(data){
  ...
  $("#engine_link").remove();
  $("#tab_engine").prepend(`<a id='engine_link' href='https://engine.otasync.me/?${main_lcode}' target='_blank'> Engine preview </a>`)
