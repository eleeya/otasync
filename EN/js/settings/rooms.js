      ...
      $("#room_settings_images").append(`
        <div class='room_image link'>
          <div class='room_image_container'>
            ...
            <div class="room_image_cancel"> Remove </div>
          </div>
        </div>`);
    }
    let image_id = new Date() / 1; // ID of each img has to be different
...
$("#all_rooms_amenities_confirm").click(function(){
  ...
  $.ajax({
    ...
    success: function(rezultat){
      ...
      add_change("Inserted amenities for all rooms", -1); // Add changelog
      get_rooms(); // Refresh data
    },
    error: function(xhr, textStatus, errorThrown){
      $(".button_loader").removeClass("button_loader");
...
$("#new_extra_predefined").change(function(){ // Add predefined values
  ...
  if(val == -1){
    ...
    $("#new_extra_name").val("Airport transport");
    $("#new_extra_variation").val(1).change();
    $("#new_extra_price").val(0);
    $("#new_extra_tax").val(0);
    $("#new_extra_type").val("one").change();
    ...
    $("#new_extra_name").val("Airport transport (both ways)");
    $("#new_extra_variation").val(1).change();
    $("#new_extra_price").val(0);
    $("#new_extra_tax").val(0);
    $("#new_extra_type").val("one").change();
    ...
    $("#new_extra_name").val("Boat transport");
    $("#new_extra_variation").val(1).change();
    $("#new_extra_price").val(0);
    $("#new_extra_tax").val(0);
    $("#new_extra_type").val("one").change();
    ...
    $("#new_extra_name").val("Club ticket");
    $("#new_extra_variation").val(1).change();
    $("#new_extra_price").val(0);
    $("#new_extra_tax").val(0);
    $("#new_extra_type").val("one").change();
...
$("body").on("click", "#new_extra_confirm", function(){  // Insert new
  ...
  $.ajax({
    ...
    success: function(rezultat){
      ...
      add_change(`Inserted extra ${name}`, sve.data.id); // Add changelog
    ...
    $(`#${row_id}`).after(`
      <div id='edit_extra_container'>
        ...
        <div class='flex_between'>
          ...
          <div class='flex_center'>
            <div class='image_input_container' id='edit_extra_image_container'>
              ...
              <div class='image_input_cancel'>Remove</div>
            </div>
        ...
        <div class='flex_center'>
          ...
          <button class='confirm_button' id='edit_extra_confirm'> SAVE </button>
        </div>
      </div>`);
      // Add values
...
$("body").on("click", "#edit_extra_confirm", function(){
  ...
  $.ajax({
    ...
    success: function(rezultat){
      ...
      add_change(`Edited extra ${name}`, sve.data.id); // Add changelog
      get_extras(); // Refresh data
    },
    error: function(xhr, textStatus, errorThrown){
      $(".button_loader").removeClass("button_loader");
...
$("#extras_list").on("click", ".delete", function(){ // Show dialog and delete
  ...
  id = id[id.length - 1];
  let extra = extras_map[id];
  if(confirm(`Are you sure you want to delete extra ${extra.name}`)){
    ...
    $.ajax({
      ...
      success: function(rezultat){
        ...
        add_change(`Deleted extra ${extra.name}`, sve.data.id); // Add changelog
        get_extras(); // Refresh data
      },
      error: function(xhr, textStatus, errorThrown){
        window.alert("An error occured. " + xhr.responseText);
...
$("#new_channel_confirm").click(function(){
  ...
  $.ajax({
    ...
    success: function(rezultat){
      ...
      add_change(`Inserted channel ${name}`, sve.data.id); // Add changelog
  ...
  if($("#edit_channel_container").length && $(`#${row_id}`).hasClass("selected")){ // This plan edit is open
    ...
    $(`#${row_id}`).after(`
      <div id='edit_channel_container'>
        ...
        <div class='flex_center'>
          <button class='cancel_button' id='edit_channel_cancel'> CANCEL </button>
          <button class='confirm_button' id='edit_channel_confirm'> SAVE </button>
        </div>
      </div>`);
...
$("body").on("click", "#edit_channel_confirm", function(){
  ...
  $.ajax({
    ...
    success: function(rezultat){
      ...
      add_change(`Edited channel ${name}`, sve.data.id); // Add changelog
      get_channels(); // Refresh data
    },
    error: function(xhr, textStatus, errorThrown){
      $(".button_loader").removeClass("button_loader");
  ...
  if(confirm(`Are you sure you want to delete channel ${channel.name}`)){
    ...
    $.ajax({
      ...
      success: function(rezultat){
        ...
        add_change(`Deleted channel ${channel.name}`, sve.data.id); // Add changelog
        get_channels(); // Refresh data
      },
      error: function(xhr, textStatus, errorThrown){
        window.alert("An error occured. " + xhr.responseText);
...
function display_extras(){
  ...
  for(let i=0;i<extras_list.length;i++){
    ...
    if(type == "room")
      ...
      type = "One time";
    var extra_edit = `<div class='list_action'><img class='list_action_icon edit' title='Edit'> </div>`;
    var extra_delete = `<div class='list_action'><img class='list_action_icon delete' title='Delete'> </div>`;
    $("#extras_list").append(`
...
function display_channels(){
  ...
  $(".channel_select").append(`<option value='-1' class='channel_option'> <img src='img/ota/youbook.png'> Private reservation </option>`);
  ...
  for(let i=0;i<channels_list.length;i++){
    ...
    var channel_edit = `<div class='list_action'><img class='list_action_icon edit' title='Edit'> </div>`;
    var channel_delete = `<div class='list_action'><img class='list_action_icon delete' title='Delete'> </div>`;
  ...
  if(channels_list.length == 0)
    ...
    <div class="list_names">
      ...
      <div class='channel_name'> Name </div>
      <div class='channel_commission'> Commission </div>
      <div class='channel_actions'> Actions </div>
    </div>`);
};
