...
$(document).ready(function(){
  ...
  // Triggers for displaying modals
  ...
  $("body").on("change", "#res_info_guest_status", function(){
    ...
    $.ajax({
      ...
      success: function(rezultat){
        ...
        add_change(`Change saved`, sve.data.id); // Add changelog
        get_calendar();
      },
  ...
  // Display full note, since it's truncated to a set amount of characters at first
    ...
    $.ajax({
      ...
      success: function(rezultat){
        ...
        if(sve.cc_number == undefined){
          $(".cc_info").after(`<div class='cc_data'> Invalid password.</div>`);
            return;
        }
  ...
  $("body").on("click", "#res_info_noshow, #res_info_invalidcc", function(){
    click_to_hide();
    add_change_settings("Change saved");
  });
});

function open_res_info(res){

  click_to_hide(); // Hides previously opened modals first
  ...
  // Wubook addons
  for(var i=0;i<res.addons_list.length;i++){
    services_html = services_html + `<div class='res_info_room'>${res.addons_list[i].name} (${res.addons_list[i].price})</div>`;
  }
  if(res.services.length || res.addons_list.length){
    services_html = `
    <div class='res_info_item'>
      <div class='res_info_tooltip_container'>
        <img class='res_info_icon' src='img/price_tag_white.svg'>
        <div class='res_info_tooltip biggest'> Additional services </div>
      </div>
  ...
  if(res.status == 1)
  {
    var status_html = `<div class='res_info_confirmed'> Confirmed </div>`;
  ...
  {
    var status_html = `<div class='res_info_modified'> Modified </div>`;
    if(res.new_reservation_code != "")
    {
      linked_reservation_html = `
      <div class='res_info_linked_res' id='res_info_linked_res_${res.new_reservation_code}'>
        New reservation ${res.new_reservation_code}
      </div>`;
    }
  }
  else if(res.status == 5){
    var status_html = `<div class='res_info_canceled'> Canceled </div>`;
    if(res.modified_reservation != "")
    {
      linked_reservation_html = `
      <div class='res_info_linked_res' id='res_info_linked_res_${res.modified_reservation}'>
        Old reservation ${res.modified_reservation}
      </div>`;
    }
  }
  ...
  if(res.note != "")
  {
    ...
    {
      note_html = `<div class='res_info_note'><div id='res_info_note_full'> ${res.note} </div> <div id='res_info_note_short'> ${res.note.substring(0,180)}... </div>  </div> <div id='res_info_note_show'> SHOW ALL </div>`;
    }
    else {
  ...
  // Guest
  var country_html = "";
  let guest_html = `<div> ${res.customer_name} ${res.customer_surname} </div>`; // In case of guests array being empty
  // Appends entire lists of guests
  if(res.guests.length > 0){
      ...
      if(res.guests.length > 1){
        guest_html += `<div id='res_info_other_guests' class='res_info_gray'> SHOW ALL </div>`;
      }
  }
  // Adds guests to global array so their info can be opened
  ...
  if(res.customer_phone !== "")
    ...
    <div class='res_info_item'>
      <div class='res_info_tooltip_container'>
        ...
        <div class='res_info_tooltip'> Phone </div>
      </div>
      <div class='res_info_text'> ${res.customer_phone} </div>
    </div>`;

  // Link to channel reservation
  var link_html = "";
  var booking_html = "";
  if(channels_map[res.id_woodoo] !== undefined)
  {
    ...
    else if(channels_map[res.id_woodoo].ctype == 2) //Booking
    {
      ...
      <div class='booking_flex'>
        ...
        <button class='update_button invalid_cc' id='invalid_cc_${res.reservation_code}'>Invalid CC</button>
      </div>
      `;
    }
  }

  // Showing inputs for fetching cc
  var cc_html = "";
  if(res.cc_info == "1")
  {
    if(wspay_active == 1)
    {
      ...
      <div class='info_center cc_info'>
        <input type='password' id='cc_pass' class='form_input' placeholder='CC password'>
        <input type='number' id='cc_payment_amount' class='number_input' placeholder='AMOUNT'>
        <button class='update_button make_cc_payment' id='cc_make_payment_${res.reservation_code}'>MAKE PAYMENT</button>
      </div>
      `;
    }
    else {
      ...
      <div class='flex_around cc_info'>
        <input type='password' id='cc_pass' class='text_input' placeholder='CC password'>
        <button class='confirm_button display_cc' id='display_cc_${res.reservation_code}'>SHOW</button>
      </div>
      `;
    }
  ...
  if(res.status == 1)
  {
    guest_status_html = `<select class='basic_select' id='res_info_guest_status'>
      ...
      <option value='arrived_and_paid'> Arrived and paid </option>
  ...
  $("body").append(`
    <div class='info' id='res_info'>
      ...
      <div class='flex_around'>
        <div class='res_info_section'>
          ...
          <div class='res_info_tooltip_container'>
            ...
            <div class='res_info_tooltip big'> Date/Time </div>
          </div>
          ...
          <div class='res_info_item'>
            <div class='res_info_tooltip_container'>
              ...
              <div class='res_info_tooltip bigger'> Period </div>
            </div>
          ...
          <div class='flex_start'>
            <div class='res_info_item'>
              <div class='res_info_tooltip_container'>
                ...
                <div class='res_info_tooltip big'> Num. of nights </div>
              </div>
            ...
            <div class='res_info_item'>
              <div class='res_info_tooltip_container'>
                <img class='res_info_icon' src='img/price_tag_white.svg'>
                <div class='res_info_tooltip bigger'> Price per night </div>
              </div>
          ...
          <div class='flex_start'>
            <div class='res_info_item'>
              <div class='res_info_tooltip_container'>
                ...
                <div class='res_info_tooltip'> Guests count </div>
              </div>
            ...
            <div class='res_info_item'>
              <div class='res_info_tooltip_container'>
                ...
                <div class='res_info_tooltip big'> Adults </div>
              </div>
            ...
            <div class='res_info_item'>
              <div class='res_info_tooltip_container'>
                ...
                <div class='res_info_tooltip'> Children </div>
              </div>
        ...
        <div class='res_info_section'>
          <div class='res_info_item'>
            <div class='res_info_tooltip_container'>
              ...
              <div class='res_info_tooltip biggest'> Rooms </div>
            </div>
          ...
          <div class='res_info_item'>
            <div class='res_info_tooltip_container'>
              ...
              <div class='res_info_tooltip big'> Total price </div>
            </div>
      ...
      <div class='res_info_buttons'>
        ...
        <button class='confirm_button' id='res_info_invalidcc'> Invalid CC </button>
  ...
  // Remove delete option for channel reservations
...
function open_guest_info(guest_id)
{
  ...
  click_to_hide(); // Hide previously opened modals
    ...
    <div class='info' id='guest_info'>
      ...
      <div class='flex_around'>
        ...
        <div class='res_info_section'>
          <div class='res_info_item'>
            <img class='res_info_icon' src='img/bed_white.svg'>
            <div class='res_info_text'> ${guest.total_arrivals} arrivals</div>
          </div>
          <div class='res_info_item'>
            <img class='res_info_icon' src='img/night_white.svg'>
            <div class='res_info_text'> ${guest.total_nights} nights </div>
          </div>
          <div class='res_info_item'>
            <img class='res_info_icon' src='img/price_tag_white.svg'>
      ...
      <div class='res_info_buttons'>
        <button class='confirm_button' id='guest_info_register'> Register arrival </button>
        <button class='confirm_button' data-value='${guest.id}' id='guest_info_edit'> Edit </button>
      </div>
    </div>
    `);
  var scroll = $("html").scrollTop() > $("body").scrollTop() ? $("html").scrollTop() : $("body").scrollTop();
  ...
  // Fetch reservations for selected guest and display them
  $.ajax({
