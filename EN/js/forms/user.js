  ...
  $.ajax({
    ...
    success: function(rezultat){
      ...
      if(id == "")
        ...
        add_change(`Izmenjen korisnik ${name}`, sve.data.id); // Add changelog
...
$("#form_user_prices").change(function(){
  ...
  if(val == 0)
    $("#form_user_prices_tooltip").text("Korisnik neće moći da vidi cenovnike.");
  if(val == 1)
    $("#form_user_prices_tooltip").text("Korisnik će moći da vidi, ali ne i da menja cenovnike.");
  if(val == 2)
    $("#form_user_prices_tooltip").text("Korisnik će moći da vidi i dodaje cenovnike, ali ne i da ih menja i briše (osim onih koje je lično uneo).");
  if(val == 3)
    $("#form_user_prices_tooltip").text("Korisnik će imati puna prava da vidi i menja sve cenovnike.");
});
