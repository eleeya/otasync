...
$("body").on("click", "#invoices_list .switch_button", function(){ // Show dialog and delete
  ...
  $.ajax({
    ...
    success: function(rezultat){
      ...
      add_change(`Izmjenjen račun ${invoices_map[id].mark}`, sve.data.id); // Add changelog
    },
...
$("#invoices_list").on("click", ".delete", function(){ // Show dialog and delete
  ...
  if(confirm(`Da li želite da obrišete račun ${invoice.mark}`)){
    $.ajax({
      type: 'POST',
      ...
      success: function(rezultat){
        ...
        add_change(`Obrisan račun ${invoice.mark}`, sve.data.id); // Add changelog
        invoices_page = 0;
...
function display_invoices(data){ // Check if destructuring works
  ...
  if(invoices_list.length == 0 && invoices_page == 1){
    $("#invoices_list").append(empty_html("Nema računa"));
  }
...
function invoice_html(loc, invoice, row_number) {
  ...
  var type = "<div>Račun</div>";
  if(invoice.type == 1)
    type = "<div>Predračun</div>";
  ...
  var invoice_edit = `<div class='list_action'><img class='list_action_icon edit' title='Izmjeni' id='invoice_edit_${id}'> </div>`;
  var invoice_send = `<div class='list_action'><img class='list_action_icon mail' title='Pošalji'> </div>`;
  var invoice_delete = `<div class='list_action'><img class='list_action_icon delete' title='Obriši' id='invoice_delete_${id}'> </div>`;
  let currency = "EUR";
