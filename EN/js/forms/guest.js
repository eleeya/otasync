...
$("#form_guest_confirm").click(function(){
  ...
  $.ajax({
    ...
    success: function(rezultat){
      ...
      if(id == "")
        add_change(`Inserted guest ${name} ${surname}`, sve.data.id); // Add changelog
      else
        add_change(`Edited guest ${name} ${surname}`, sve.data.id); // Add changelog
...
function open_guest_form(guest){
  $("#form_guest h1").text("Update guest");
