                          ...
                          $("#articles").append(`
                            <div class='list_row article'>
                              ...
                              <div class='article_actions'>
                                <div class='list_action' onclick="show_edit_article_form('${article.id}')"><img class='list_action_icon edit' title='Edit'> </div>
                                <div class='list_action' onclick="delete_article('${article.id}')"><img class='list_action_icon delete' title='Delete'> </div>
                              </div>
                            </div>
                            `);
                        });
                        if(articles.length){
                          $("#articles").prepend(`
                            <div class="list_names">
                              ...
                              <div class='article_tax_rate'> Tax </div>
                              <div class='article_description'> Description </div>
                              <div class='article_price'> Period </div>
                              <div class='article_actions'> Actions </div>
                            </div>
                            `);
                        }
                        else {
                          $("#articles").append(empty_html("No articles"));
                        }
                    },
                    error: function(xhr, textStatus, errorThrown){
                      window.alert("An error occured. " + xhr.responseText);
...
function edit_article() {
    ...
    $.ajax({
        ...
        success: function(rezultat){
          ...
          add_change(`Edited article ${description}`, sve.data.id);
          var tree = $("#categories").jstree(true);
          tree.deselect_all();
          tree.select_node(node.id);
