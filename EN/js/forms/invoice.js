...
$("#form_invoice_new_service").click(function(){
  ...
  $("#form_invoice_services").append(`
    <div class='flex_around form_invoice_service'>
      <div class='form_invoice_service_name'> <input type='text' class='text_input form_invoice_service_name_input' placeholder='Name'> </div>
...
$("#form_invoice_add_article").change(function(){
  ...
  $("#form_invoice_services").append(`
    <div class='flex_around form_invoice_service'>
      ...
      <div class='form_invoice_service_name'> <input type='text' class='text_input form_invoice_service_name_input' placeholder='Name' value='${article.description}'> </div>
      <div class='form_invoice_service_amount'> <input type='number' class='number_input form_invoice_service_amount_input' value=1> </div>
...
$("#new_invoice").click(function(){
  ...
  if(property.pdv_included == 0)
    $("#form_invoice_pdv").text("The taxpayer isn't in the VAT system");
  else
    $("#form_invoice_pdv").text("The taxpayer is in the VAT system");
...
$("#form_invoice_confirm").click(function(){
  ...
  $.ajax({
    ...
    success: function(rezultat){
      ...
      if(id == "")
        add_change(`Inserted invoice ${mark}`, sve.data.id); // Add changelog
      else
        add_change(`Edited invoice ${mark}`, sve.data.id); // Add changelog
...
function open_res_invoice(res_id){
  ...
  for(let i=0;i<res.room_data.length;i++){
    ...
    $("#form_invoice_services").append(`
      <div class='flex_around form_invoice_service'>
        ${code}
        <div class='form_invoice_service_name'> <input type='text' class='text_input form_invoice_service_name_input' placeholder='Name' value='${room.name}'> </div>
...
function open_invoice_form(invoice){
  ...
  for(let i=0;i<invoice.services.length;i++){
    ...
    $("#form_invoice_services").append(`
      <div class='flex_around form_invoice_service'>
        ${code}
        <div class='form_invoice_service_name'> <input type='text' class='text_input form_invoice_service_name_input' placeholder='Name' value='${service.name}'> </div>
