...
$("#reservations_list").on("click", ".delete", function(e){ // Show dialog and delete
  ...
  let confirm_text = `Are you sure you want to cancel reservation ${reservation.reservation_code}`;
  if(reservation.status == 5)
    confirm_text = `Are you sure you want to delete reservation ${reservation.reservation_code}`;
  if(confirm(confirm_text)){
    $.ajax({
      ...
      success: function(rezultat){
        ...
        let change_text = `Canceled reservation ${reservation.reservation_code}`;
        if(reservation.status == 5)
          change_text = `Deleted reservation ${reservation.reservation_code}`;
...
$("body").on("click", ".res_info_delete", function(e){ // Show dialog and delete
  ...
  let confirm_text = `Are you sure you want to cancel reservation ${reservation.reservation_code}`;
  if(reservation.status == 5)
    confirm_text = `Are you sure you want ot delete reservation ${reservation.reservation_code}`;
  click_to_hide();
  if(confirm(confirm_text)){
    $.ajax({
      type: 'POST',
...
function display_reservations(data){
  ...
  if(reservations_list.length == 0 && reservations_page == 1)
    $("#reservations_list").append(empty_html("No reservations"));
  else if(reservations_page == 1)
    $("#reservations_list").prepend(`
    <div class="list_names">
      <div class='res_channel'> Channel </div>
      <div class='res_received'> Date </div>
      <div class='res_guest'> Guest </div>
      <div class='res_period'> Period </div>
      <div class='res_price'> Amount </div>
      <div class='res_rooms'> Rooms </div>
      <div class='res_note'> Note </div>
      <div class='res_actions'> Actions </div>
    </div>`);
}
...
function res_html(loc, res){
  ...
  var status = `<div class='res_confirmed'> Confirmed </div>`;
  if(res.status == 5 && res.was_modified == 0)
    status = `<div class='res_canceled'> Canceled </div>`;
  else if(res.status == 5 && res.was_modified == 1)
    status = `<div class='res_modified'> Modified </div>`;
  ...
  let res_edit = `<div class='list_action'><img class='list_action_icon edit' title='Edit'> </div>`;
  let res_invoice = `<div class='list_action'><img class='list_action_icon invoice' title='Invoice'> </div>`;
  let res_delete = `<div class='list_action'><img class='list_action_icon delete' title='Delete'> </div>`;
