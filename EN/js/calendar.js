...
var scroll_clicked = false; // Used for calendar drag to scroll
// Used for saving tetris values (on reservation move in calendar)
var click_vals =
{
  dfrom: "",
  dto: "",
    ...
    else if(tetris_vals.dfrom !== "")
    {
      ...
      if(open_info !== "") { // open_info gets reset on timer, this opens the info if the timer hasn't triggered yet
        ...
        if(tetris_org.room !== tetris_vals.room || tetris_org.room_number !== tetris_vals.room_number){
          ...
          $("#cal_dialog_changes").append("<div> Room: </div>")
  ...
  $("#cal_dialog_confirm").click(function(){
    ...
    $.ajax({
      ...
      success: function(rezultat){
        ...
        add_change(`Edited reservation ${tetrisAjaxData.id}`, sve.data.id); // Add changelog
...
function send_room_status(status, room, room_number){
  $.ajax({
    ...
    success: function(rezultat){
      ...
      add_change("Change saved", sve.data.id);
...
const month_names = [0,"January","February","March","April","May","June","July","August","September","October","November","December"];
const day_names = ["NE","PO","UT","SR","ČE","PE","SU"];

// Displays empty calendar
...
// Apending avail/price/min to calendar
...
// Appending reservations to calendar
function display_calendar_reservations(data){
  ...
  for(var i=0;i<reservations.length;i++){
    ...
    if(channels_map[res_channel] !== undefined)
      ...
      res_channel = "Private reservation";
    // Name and Info
    var channel_img = "";
            ...
            $(`#cal_row_${res_room}_${rooms_map[res_room].availability - 1} .cal_label`).append(`
              <div class='cal_error' id='cal_error_${res_room}'>
                <div class='cal_error_tooltip'>
                  There are more reservations in this room type than total number of rooms. Click to display those reservations.
                </div>
              </div>`);
          }
...
// Functions for updating values from calendar
